...
void MainThread::search() {
  ...
  //from Sugar
  limitStrength = Options["UCI_LimitStrength"] || Options["Use_Personality"] ;
  //end from Sugar
  ...
  TT.new_search();
  opening = Options["Choices Structure"];//from Sugar
  Eval::init(true);
  //from Shashin
  highTal=Options["Highly Attack"];
  mediumTal=Options["Aggressiveness"];
  lowTal=Options["Positional"];
  capablanca=Options["Strategy"];
  highPetrosian=Options["Highly defensive"];
  mediumPetrosian=Options["Conservative"];
  lowPetrosian=Options["Accurate"];
  //end from Shashin
  //from handicap mode begin
  uciElo=Options["UCI_LimitStrength"] ? Options["UCI_Elo"]:Options["Elo_Rating"];
  depthLimit=limitStrength && Options["Personality Depth"] ? getHandicapDepth(uciElo):Limits.depth;//handicap mode
  ...
  else
  {
      ...
      {
	        if(Options["Knowledge_Calibration"])
          Limits.nodes = int(Options["Knowledge_Calibration"]);
 
      if ( Options["Calculation_Skills"])
          depthLimit= int(Options["Calculation_Skills"]);
...
void Thread::search() {
  ...
  if (multiPV == 1)    {
              corr = Options["Blindness"];
      	      corr_depth = Options["Depth Lines"];
    }
  else  corr = 0;

  variety = Options["Less Moves Waited"];

  if (corr && !variety ) multiPV = size_t(pow(2, corr));
  ...
  int ct;
  //int ct = int(Options["CPuct"]) * PawnValueEg / 100; // From centipawns
  if (Options["Select Style"] == "None") ct = 0 * PawnValueEg / 100;
  if (Options["Select Style"] == "Aggressiveness") ct = 65 * PawnValueEg / 100;
  if (Options["Select Style"] == "Ultra-Aggressiveness") ct = 100 * PawnValueEg / 100;
  if (Options["Select Style"] == "Ultra-Defensive") ct = -100 * PawnValueEg / 100;
  if (Options["Select Style"] == "Positional") ct = 24 * PawnValueEg / 100;
  if (Options["Select Style"] == "Strategy") ct = 8 * PawnValueEg / 100;
  if (Options["Select Style"] == "Defence") ct = 0 * PawnValueEg / 100;
  if (Options["Select Style"] == "Universal") ct = 12 * PawnValueEg / 100;
/*
...
if(!Options["Tactical Combinations"]){
MagicTacticSolver = 1;}
else if(Options["Tactical Combinations"]){
...
int pehh = int(Options["Pawn Exchange Rate"])/16;
int kehh = int(Options["Knight Exchange Rate"])/4;
int behh = int(Options["Bishop Exchange Rate"])/4;
int rehh = int(Options["Rook Exchange Rate"])/4;
int qehh = int(Options["Queen Exchange Rate"])/2;
...
Qex = (us == WHITE ?  make_score(qehh, qehh / 2)
  ...
  int failedAspirationsCurrentID = 0; //aspiration patch
	...
	//aspiration patch end
      ...
      if (corr  && rootDepth > corr_depth )
           multiPV = 1;
      else if  (variety && !variety_flag && !opening) multiPV = 1;
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
	    ...
	    if (rootDepth >= 4)
	    {
			...
			updateShashinValues(rootPos,prev);//from ShashChess
              ...
              int dct = ct + (113 - ct / 2) * prev / (abs(prev) + 147 + defensive_v);
	...
	//from true handicap mode end
          ...
          && !mainThread->stopOnPonderhit)
      {
          ...
          if (Options["UCI_LimitStrength"])
              Stockfish::Search::Limits.nodes = (Options["Brain_Calculations"]) *totalTime;
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) { 
    ...
    bool givesCheck, improving, didLMR, priorCapture, isMate; //from Crystal
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if ( doLMRStep &&   depth >= 2 &&  moveCount
...
// ttValue can be used as a better position evaluation (~7 Elo)
    ...
    //from Sugar
    if (opening && (bestValue + (opening * PawnValueEg / 100) >= 0 ) && (pos.count<PAWN>() > 12))
	  bestValue += rand() % (opening + 1);
    //end from Sugar
