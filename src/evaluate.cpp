...
namespace Eval {
  ...
  void NNUE::init() {
    ...
    useNNUE = Options["Use NNUE Evaluation"]&&(!Options["UCI_LimitStrength"])&&(!Options["LimitStrength_CB"]);
    if (!useNNUE)
        return;
...
#undef S
  ...
  Score Evaluation<T>::threats() const {
    ...
    Bitboard b, bb, weak, defended, nonPawnEnemies, stronglyProtected, safe, targets;
    ...
    if (pos.count<QUEEN>(Them) == 1)
    {
		...
		while (targets)
		{
			bb = targets & (targets ^ (targets - 1));
			targets ^= bb;

			defense =    (bb & attackedBy[Them][KNIGHT])
					   + (bb & attackedBy[Them][BISHOP])
					   + (bb & attackedBy[Them][ROOK])
					   + (bb & attackedBy[Them][QUEEN])
					   + (bb & attackedBy[Them][KING]);

			attack  =    (bb & attackedBy[Us][PAWN])
					   + (bb & attackedBy[Us][KNIGHT]) 
					   + (bb & attackedBy[Us][BISHOP])
					   + (bb & attackedBy[Us][ROOK])
					   + (bb & attackedBy[Us][QUEEN])
					   + (bb & attackedBy[Us][KING]);
  ...
  pieces_A = Options["Initiative White"];
  pieces_B = Options["Initiative Black"];
  ...
  Value Evaluation<T>::value() {
    ...
    Score score =  apply_weight(pos.psq_score(), Weights[MaterialT])
	             + apply_weight(me->imbalance() + pos.this_thread()->MagicTacticSolver + pos.this_thread()->contempt, Weights[Imbalance]);
    // Probe the pawn hash table
    pe = Pawns::probe(pos);
    if (pawnsToEvaluate)
    {
    score += apply_weight(pe->pawn_score(WHITE), Weights[PawnStructure]);
	         apply_weight(pe->pawn_score(BLACK), Weights[PawnStructure]);
    }
...
make_v:
    ...
    if constexpr (T)
    {
      ...
      Trace::add(IMBALANCE, apply_weight(me->imbalance() + pos.this_thread()->contempt, Weights[Imbalance]));
      Trace::add(PAWN, apply_weight(pe->pawn_score(WHITE), Weights[PawnStructure]),
	                   apply_weight(pe->pawn_score(BLACK), Weights[PawnStructure]));
      ...
      if (pos.non_pawn_material(WHITE) + pos.non_pawn_material(BLACK) >= 12317)
          Trace::add(SPACE, apply_weight(space<  WHITE>(), Weights[Space])
                          , apply_weight(space<  BLACK>(), Weights[Space]));
...
Value Eval::evaluate(const Position& pos) {
  ...
  v = (Eval::RandomEvalPerturb * Value(r) + (100 - Eval::RandomEvalPerturb) * v) / 100;
...
void init() {
      ...
      Weights[MaterialT]     = { (int)Options["Material(MG)"], (int)Options["Material(EG)"] };
      Weights[Imbalance]     = { (int)Options["Imbalance(MG)"], (int)Options["Imbalance(EG)"] };
      Weights[PawnStructure] = { (int)Options["PawnStructure(MG)"], (int)Options["PawnStructure(EG)"] };
      Weights[Mobility]      = { (int)Options["Mobility(MG)"], (int)Options["Mobility(EG)"] };
      Weights[PassedPawns]   = { (int)Options["PassedPawns(MG)"], (int)Options["PassedPawns(EG)"] };
      Weights[KingSafety]    = { (int)Options["KingSafety(MG)"], (int)Options["KingSafety(EG)"] };
      Weights[Threats]       = { (int)Options["Threats(MG)"], (int)Options["Threats(EG)"] };
      Weights[Space]         = { (int)Options["Space(MG)"],(int) Options["Space(EG)"] }; 
    // SPSA
    MultiAttack      = make_score(int(Options["MultiAttack(mg)"])  , int(Options["MultiAttack(eg)"]));
    Hanging          = make_score(int(Options["Hanging(mg)"])      , int(Options["Hanging(eg)"]));
	FlankAttacks = make_score(int(Options["FlankAttacks(mg)"]), int(Options["FlankAttacks(eg)"]));
	  }
  } // namespace Stockfish
}
